...
export default function PricingSection() {
  const plans = [PRICING_CONFIG.free, PRICING_CONFIG.pro, PRICING_CONFIG.enterprise, PRICING_CONFIG.enterpriseUltra];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8 mb-8">
          {plans.map((plan) => {
            ...
            const isEnterpriseUltra = plan.id === 'enterprise-ultra';
            ...
            return (
              <div
                ...
                className={`relative rounded-2xl p-8 transition-all hover:shadow-2xl ${
                  isEnterpriseUltra
                    ? 'bg-gradient-to-br from-purple-50 via-indigo-50 to-violet-50 border-2 border-purple-500 shadow-xl'
                    : isEnterprise
              ...
              >
                ...
                <h3 className={`text-2xl font-bold mb-2 ${
                  isEnterpriseUltra ? 'text-purple-900' : isEnterprise ? 'text-amber-900' : 'text-gray-900'
                }`}>
                  {plan.name}
                </h3>
                ...
                <div className="mb-4">
                  ...
                  <span className={`ml-1 ${
                    isEnterpriseUltra ? 'text-purple-700' : isEnterprise ? 'text-amber-700' : 'text-gray-600'
                  }`}>
                    /{plan.period}
                  </span>
                </div>
                ...
                <p className={`mb-6 text-sm ${
                  isEnterpriseUltra ? 'text-purple-800' : isEnterprise ? 'text-amber-800' : 'text-gray-600'
                }`}>
                  {plan.description}
                </p>
                ...
                <ul className="space-y-3 mb-8">
                  {plan.features.slice(0, 5).map((feature, fIndex) => (
                    <li key={fIndex} className="flex items-start gap-2 text-sm">
                      <span className={`mt-0.5 ${
                        isEnterpriseUltra ? 'text-purple-500' : isEnterprise ? 'text-amber-500' : 'text-emerald-600'
                      }`}>✓</span>
                      <span
                        className={isEnterpriseUltra ? 'text-purple-900' : isEnterprise ? 'text-amber-900' : 'text-gray-700'}
                  ...
                  {plan.features.length > 5 && (
                    <li className={`text-sm ${
                      isEnterpriseUltra ? 'text-purple-700' : isEnterprise ? 'text-amber-700' : 'text-gray-500'
                    }`}>
